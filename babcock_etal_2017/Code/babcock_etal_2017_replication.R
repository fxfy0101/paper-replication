...
packages <- c("tidyverse", "haven", "AER", "mfx", "RCT", "stargazer", "texreg")
...
### balance tables -------------------------------------------------------------------
### three balance tables, one for each experiment
### The balance tables summarize t tests on the equality of means of different
### characteristics sub-grouped by gender.

### balance table for experiment 1
bal_tab_exp_1 <- experiments_1_2 %>%
  filter(experiment == 1) %>%
  distinct(unique_subjectid, .keep_all = TRUE) %>%
  dplyr::select(female:freshman_soph) %>%
  balance_table("female") %>%
  rename(covariates = variables1, male = Media_control1, female = Media_trat1, p_value = p_value1) %>%
  as.data.frame()

### balance table for experiment 2  
bal_tab_exp_2 <- experiments_1_2 %>%
  filter(experiment == 2) %>%
  distinct(unique_subjectid, .keep_all = TRUE) %>%
  dplyr::select(female:freshman_soph) %>%
  balance_table("female") %>%
...
### balance table for experiment 3
bal_tab_exp_3 <- experiment_3 %>%
  distinct(unique_subjectid, .keep_all = TRUE) %>%
  dplyr::select(female, race:born_us) %>%
  balance_table("female") %>%
  rename(covariates = variables1, male = Media_control1, female = Media_trat1, p_value = p_value1) %>%
  as.data.frame()
...
### remove all objects in the current R environment --------------------------------
### DO NOT RUN IF YOU DO NOT WANT TO REMOVE OBJECTS
rm(list = ls())
